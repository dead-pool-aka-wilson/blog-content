...
[이전 글](/ko/posts/2026-02-02-ai-blog-series-1-start/)에서 콘텐츠 시드 시스템을 소개했다. prompts, ideas, 그리고 **misses**. 실수를 기록하는 카테고리다.
...
문제가 보이는가? 현재 사용자는 `newuser`인데, 심링크는 `/Users/olduser/`를 가리키고 있다. 예전 맥에서 설정한 경로가 그대로 남아있었다.
...
다음 글에서는 이 모든 과정에서 발견한 흥미로운 패턴에 대해 이야기하겠다.
